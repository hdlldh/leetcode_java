...
class Solution {
    ...
    private int helper(int[] nums, int l, int r, HashMap<Integer, Integer> mem){
        if (l==r) return nums[l];
        int n = nums.length;
        if (mem.containsKey(n*l+r)) return mem.get(n*l+r);
        int ans = Math.max(nums[l]-helper(nums, l+1, r, mem), nums[r]-helper(nums, l, r-1, mem));
        mem.put(n*l+r, ans);
        return ans;
    }
}
