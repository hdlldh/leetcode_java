...
class Solution {
    public boolean stoneGame(int[] piles) {
        ...
        return helper(piles, 0, n-1, mem) > 0;
    }
    private int helper(int[] piles, int i , int j, int[][] mem){
        if (i>j) return 0;
        if (i==j) return piles[i];
        if (mem[i][j] >0) return mem[i][j];
        int ans = 0;
        ans = Math.max(piles[i] - helper(piles, i+1, j, mem), piles[j] -helper(piles, i, j-1, mem));
        mem[i][j] = ans;
        return ans;
    }
}
