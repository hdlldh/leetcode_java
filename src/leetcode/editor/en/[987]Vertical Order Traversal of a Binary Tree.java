...
class Solution {
    public List<List<Integer>> verticalTraversal(TreeNode root) {
        ...
        PriorityQueue<int[]> pq = new PriorityQueue<>(Comparator.comparing((int[] x)->x[0]).thenComparing((x)->x[1]).thenComparing((x)->x[2]));
        dfs(root, 0, 0, pq);
        int[] p = pq.peek();
        int level = p[0];
    ...
    private void dfs(TreeNode root, int x, int y, PriorityQueue<int[]> pq){
        if (root==null) return;
